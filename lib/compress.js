...
Compress.prototype.main = function(){
    // 解析模板文件
    ...
    for (var i = 0, count = analyticContent.length; i < count; i++){
        ...
        switch (tokenType){
            // 去掉换行符
            case util.sign.FL_NEW_LINE :
                break;
            // html 注释 去掉
            case util.sign.HTML_COMMENT : 
                break;
            // html 文本内容
            ...
            case util.sign.HTML_JS_CONTENT :
                this._comressScriptContent();
                break;

            // tpl 模板标签
            case util.sign.FL_TPL_DELIMITER:
                var text = this._tokenText;
                if (text.indexOf(this.leftDelimiter + '*') !== -1){ //去掉注释
                    break;
                }
...
Compress.prototype._comressScriptContent = function(){
    if (this._tokenText !== ''){
        //去除注释
        var reg = /("([^\\\"]*(\\.)?)*")|('([^\\\']*(\\.)?)*')|(\/{2,}.*?(\r|\n))|(\/\*(\n|.)*?\*\/)/g;
        this._tokenText = this._tokenText.replace(reg, function(word) { // 去除注释后的文本  
            return /^\/{2,}/.test(word) || /^\/\*/.test(word) ? "" : word;  
        });

        //多空格转一个
        reg = /\s{2,}/g;
        this._tokenText = this._tokenText.replace(reg," ");

        var tmp = this._tokenText.split(/\n|\r|\t/);
        var out = [];
        for(var i = 0, l = tmp.length; i < l; i++){
            var _out = util.trim(tmp[i]);
            if(_out.substr(0, 2) == '//') {
                continue;
            }
 ...
 */
Compress.prototype._compressTextContent = function(){
    if (this._tokenText !== ''){
...
Compress.prototype._compressTag = function(){
...
Compress.prototype.isDelimiterScriptStart = function(tokenType){
...
Compress.prototype._checkEqual = function(str, start, len, result){
