 ...
 * @author  Yang,junlong at 2015-07-07 17:05:57 build.
 * @version $Id$
 */
...
module.exports = function(content, type, leftDelimiter, rightDelimiter){
    var parser = new Parser();
    return parser.main(content, type, leftDelimiter, rightDelimiter);
};
...
Parser.prototype.main = function(content, type, leftDelimiter, rightDelimiter){
    this.content = content;
    this.leftDelimiter = leftDelimiter;
    this.rightDelimiter = rightDelimiter;

    // {%script%} {%/script%} fis script block
    ...
    if (this.content.indexOf && this.content.indexOf('<?xml') !== -1){
        return new Array([content, util.sign.HTML_XML]);
    }
 ...
 * 分析字符
 */
Parser.prototype.tokenAnalytic = function(){
    while (true){
        var token = this.getNextToken();
        if (token){
            if (token[1] === util.sign.FL_EOF) {
                break;
            }
...
Parser.prototype.getNextToken = function(){
    if (this.parsePos >= this.contentLength){
            return new Array('', util.sign.FL_EOF);
        }
        ...
        if (outputCount){
            ...
            }else if (tokenType === util.sign.HTML_CSS_START){
        ...
        if (str === "\x0a"){// \n
            return new Array(str, util.sign.FL_NEW_LINE);
        }
        //处理模板左右定界符,如果是smarty变量则直接返回
        result = util.getTplDelimiterToken.call(this);
        if (result){
            //单独针对script插件类型处理
        ...
        if (result) return result;
        return new Array(str, util.sign.FL_NORMAL);
};
 ...
 * @private
 */
Parser.prototype._getTagToken = function(str){
    ...
    do {
        ...
        this.parsePos++;
        var result = util.getTplDelimiterToken.call(this);
        if (result){
    ...
    if (resultString[1] === '!'){
        if (resultString.indexOf('[if') !== -1){
            ...
            return new Array(resultString, util.sign.HTML_IE_HACK_START);
        }else if (resultString.indexOf('[endif') !== -1){
            return new Array(resultString, util.sign.HTML_IE_HACK_EDN);
        }else if (this._checkEqual(resultString, 2, 7, 'doctype')){
            return new Array(resultString, util.sign.HTML_DOC_TYPE);
        }else if(this._checkEqual(resultString, 4, 6, 'status')){
            resultString += this._getUnformated('-->', resultString);
            return new Array(resultString, util.sign.HTML_STATUS_OK);
        }else {
            resultString += this._getUnformated('-->', resultString);
            return new Array(resultString, util.sign.HTML_COMMENT);
        }
    }
    if (this._checkEqual(resultString, 0, 7, '<script')){
        return new Array(resultString, util.sign.HTML_JS_START);
    }else if (this._checkEqual(resultString, 0, 9, '</script>')){
        return new Array(resultString, util.sign.HTML_JS_END);
    }else if (this._checkEqual(resultString, 0, 6, '<style')){
        return new Array(resultString, util.sign.HTML_CSS_START);
    }else if (this._checkEqual(resultString, 0, 8, '</style>')){
        return new Array(resultString, util.sign.HTML_CSS_END);
    }
    if (this._checkEqual(resultString, 0, 2, '</')){
        return new Array(resultString, util.sign.HTML_TAG_END);
    }
    return new Array(resultString, util.sign.HTML_TAG_START);
};

Parser.prototype._checkEqual = function(str, start, len, result){
 ...
 * @private
 */
Parser.prototype._getPreTagToken = function(){
    ...
    return [resultString, util.sign.HTML_PRE_TAG];
};
 ...
 * @private
 */
Parser.prototype._getTextareaTagToken = function(){
    ...
    return [resultString, util.sign.HTML_TEXTAREA_TAG];
};
 ...
 * @private
 */
Parser.prototype._getContentToken = function(str){
    var resultString = str;
    while (true){
        if (this.parsePos >= this.contentLength){
            break;
    ...
    return [resultString, util.sign.HTML_CONTENT];
};

Parser.prototype._getUnformated = function(str, orign){
    ...
    do {
        ...
        this.parsePos++;
        var re = util.getTplDelimiterToken.call(this);
        if(re){
            resultString += re[0];
        }else{
            resultString += c;
 ...
 * @private
 */
Parser.prototype._getScriptORSTYLE = function(str, tokenText, type){
    ...
    return [resultString, type === 1 ? util.sign.HTML_JS_CONTENT : util.sign.HTML_CSS_CONTENT];
};
 ...
 * @return {Array}
 */
Parser.prototype.getTagAttributes = function(tagContent){
    //tag end
    tagContent = util.trim(tagContent + '');
    //将还行符替换为空格
    tagContent = util.str_replace(["\r\n", "\r", "\n", "\t"], ' ', tagContent);
    if (tagContent.substr(0, 2) === '</') {
        return [ 
                util.sign.HTML_TAG_END, 
                util.trim(tagContent.substr(2, tagContent.length - 3))
        ];
    }
    //tag start
    var result = [util.sign.HTML_TAG_START, '', []];
    //将最后的>和/去掉, 最多只能去一个，因为smarty的右定界符可能含有/和>
    tagContent = util.trim(tagContent.substr(0, tagContent.length - 1));
    ...
    if(!tagName) {
        str = this.content[this.parsePos];
        this.parsePos++;
        re = util.getTplDelimiterToken.call(this);
        if(re) {
            tagName = re[0];
        }else{
            this.parsePos--;
    ...
    while (true){
        ...
        if (re){
            ...
            result[2].push([util.sign.HTML_TPL_ATTR_NAME, re[0]]);
        }else if (str === '"' || str === "'"){
            //处理value="<&if test=""&>1<&else&>0<&/if&>"的情况
            this.parsePos++;
            var o = util.getTplDelimiterToken.call(this);
            re = str;
            if (o){
                re += o[0];
            }else{
            ...
            if (attr){
                ...
                if (preItem[0] === util.sign.HTML_TPL_ATTR_NAME){
                    name = preItem[1];
                    result[2].pop();
                }
            }
...
Parser.prototype.splitAttributeValue =function (value){
    ...
    var result = [];
    var item = '';
    var patternstr = util.preg_quote(this.leftDelimiter, '/') 
                  + '\\s*\\(.*?)\\s*' .util.preg_quote(this.rightDelimiter, '/');
    ...
    while (true){
        ...
        if (re){
            ...
            if (pattern.test(tpl)){//输出的字符
                ...
                result.push([tpl, util.sign.HTML_ATTR_VALUE_MIXED]);
            }else{
                if (item){
                    result.push([item, util.sign.HTML_CONTENT]);
                    item = '';
                }
                result.push(re);
            }
